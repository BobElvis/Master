...
def murty(P0, K):
    ...
    Q.put((C0, next(c), P0.copy(), S0))              # Add (C0, P0, S0) to the queue. Use time to sort equal costs
        ...
        for y, x in enumerate(S):                   # Iterating over the solution, ignoring the last (4.3)
            ...
            if valid(newP):                         # if new solution exists (4.3.4)
                ...
                if tuple(newS) not in found:        # avoid duplicate answers
                    ...
                    Q.put((newC,next(c),newP,newS))  # Add (newC, newP, newS) to the queue, use time to sort equal costs
