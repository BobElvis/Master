...
def KF_step(estimate, measurement, H, R):
    S = H.dot(estimate.cov_prior).dot(H.T)+R
    z = measurement.value
    z_hat = H.dot(estimate.est_prior)
    kalman_gain = estimate.cov_prior.dot(H.T).dot(np.linalg.inv(S))
    estimate.est_posterior = estimate.est_prior+np.dot(kalman_gain, z-z_hat)
    estimate.cov_posterior = estimate.cov_prior-np.dot(kalman_gain, np.dot(S, kalman_gain.T))
